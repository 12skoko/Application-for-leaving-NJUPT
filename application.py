...
def application(info):
    try:
        ...
        fromvariable = ['$C{sqr}', 'sqr', '$C{xh}', '$C{xy}', 'xy', '$C{zy}', 'zy', '$C{BJ}', 'BJ', '$C{lxdh}',
                        '$C{qjdmc}', '$C{sqsj}',
                        '$C{jhcxrq}', '$C{jhfxrq}', '$C{mdd}', '$C{cxsy}', '$C{jjlxr}', '$C{jjlxrdh}']

        for v in fromvariable:
            if '$' in v:
                vt = '\\' + v
            else:
                vt = v
            r0 = re.compile('<INPUT  type=\"hidden\"  name=\"' + vt + '\" id=\"' + vt + '\"  value=\"(\d*?)\" />')
            # print(r0)
            t0 = re.search(r0, req.text)
            # print(v,t0[1])
            data[v] = t0[1]
